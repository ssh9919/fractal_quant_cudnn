...
{
    /* IBM check */
    /* cudnn v2 related functions tensor create */
...
{
    /* IBM check */
    /* cudnn v2 related functions tensor destroy */
...
/* IBM check */
/* Convolution Forward part using cudnn v2 */
...
/* IBM check */
/* Convolution Backward part using cudnn v2 */
        ...
        )
{
    ...
    if(performBackwardProp == true)
    {
        checkCUDNN(cudnnConvolutionBackwardData_v2(cudnnHandle,&alpha,filterDesc,ptrweight,srcTensorDesc,ptrnextErr,convDesc,&beta,dstTensorDesc,ptrprevErr));
    memprevErr->Push(loc);
    ...
    {
    checkCUDNN(cudnnConvolutionBackwardFilter_v2(cudnnHandle,&alpha,srcTensorDesc,ptrprevAct,dstTensorDesc,ptrnextErr,convDesc,&beta,filterDesc,ptrderiv));
    memderiv->Push(loc);
    }
    /*cudaThreadSynchronize();
      cudaMemcpy(result_backward,ptrprevAct,28*28*sizeof(FLOAT),cudaMemcpyDeviceToHost);
...
/* IBM check */
/* Convolution Bias Forward part using cudnn v2 */
void Engine::ConvBiasForward(Matrix<FLOAT> &_nextLayerState, Matrix<FLOAT> &_biases, long nextLayerDimY, long nextLayerDimX, long nextLayerNumMaps, long curBatchSize, PStream &stream)
{
    ...
    checkCUDNN(cudnnAddTensor_v2(cudnnHandle,CUDNN_ADD_SAME_C,&alpha,biasTensorDesc,ptrbiases,&beta,dstTensorDesc, ptrnextState));
...
/* IBM check */
/* Convolution Bias Backward part using cudnn v2 */
...
#if 0 //IBM check convolution bias without cudnn
void Engine::ConvBiasForward(Matrix<FLOAT> &_nextLayerState, Matrix<FLOAT> &_biases, long nextLayerDimY, long nextLayerDimX, long nextLayerNumMaps, long curBatchSize, PStream &stream)
{
    verify(_biases.GetEngine() == this);
    verify(_nextLayerState.GetEngine() == this);
...
/* IBM check */
/* Max pooling or Average pooling Forward part using cudnn v2 */
void Engine::MaxPoolForward(Matrix<FLOAT> &_prevLayerAct, Matrix<FLOAT> &_nextLayerState,
        ...
        long curBatchSize, PStream &stream, long max_avg )
{
    verify(_prevLayerAct.GetEngine() == this);
    verify(_nextLayerState.GetEngine() == this);
...
/* IBM check */
/* Max pooling or Average pooling Backward part using cudnn v2 */
...
/* IBM check start */
/* Signal quantization for Sigmoid */
void Engine::FuncSigmoid(Matrix<FLOAT> &X, Matrix<FLOAT> &Y, Matrix<FLOAT> &Y_fixed, PStream &stream, FLOAT delta)
{
    verify(X.GetEngine() == this);
    verify(Y.GetEngine() == this);
    verify(Y_fixed.GetEngine() == this);
    verify(X.GetNumRows() == Y.GetNumRows());
    verify(X.GetNumCols() == Y.GetNumCols());

    FLOAT *ptrX, *ptrY, *ptrY_fixed;

    ptrX = X.GetPtrForReadWrite(stream);
    ptrY = Y.GetPtrForReadWrite(stream);
    ptrY_fixed = Y_fixed.GetPtrForReadWrite(stream);
#ifdef FRACTAL_USE_CUDA
    cudaKernels::FuncSigmoid(ptrX, ptrY, ptrY_fixed, Y.GetNumRows() * Y.GetNumCols(), stream.cudaStream, delta);
#else
    verify(false); /* CPU computation is not supported */
#endif /* FRACTAL_USE_CUDA */
	Y.FinishWrite(stream);
	Y_fixed.FinishWrite(stream);
#if 0//QUANT_DIRECT	
...
/* IBM check start */
/* Weight quantization */
void Engine::WeightQuant(Matrix<FLOAT> &X, Matrix<FLOAT> &Y, PStream &stream,FLOAT delta,int M)
{
    verify(X.GetEngine() == this);
    verify(Y.GetEngine() == this);
    verify(X.GetNumRows() == Y.GetNumRows());
...
#ifdef FRACTAL_USE_CUDA
    cudaKernels::WeightQuant(ptrX, ptrY, Y.GetNumRows() * Y.GetNumCols(), stream.cudaStream, delta, M);
#else
    verify(false); /* CPU computation is not supported */
#endif /* FRACTAL_USE_CUDA */
...
/* IBM check start */
/* Signal quantization for RectLinear */
void Engine::FuncRectLinear(Matrix<FLOAT> &X, Matrix<FLOAT> &Y, Matrix<FLOAT> &Y_fixed,PStream &stream, FLOAT delta, int M,int relu_delta_final_decision)
{
    verify(X.GetEngine() == this);
    verify(Y.GetEngine() == this);
    verify(Y_fixed.GetEngine() == this);
    ...
    ptrX = X.GetPtrForReadWrite(stream);
    ptrY = Y.GetPtrForWrite(stream);
    ptrY_fixed = Y_fixed.GetPtrForWrite(stream);

#ifdef FRACTAL_USE_CUDA
    cudaKernels::FuncRectLinear(ptrX, ptrY, ptrY_fixed, Y.GetNumRows() * Y.GetNumCols(), stream.cudaStream, delta, M,relu_delta_final_decision);
#else
    verify(false); /* CPU computation is not supported */
#endif /* FRACTAL_USE_CUDA */
...
void Engine::GenerateDropoutMask(Matrix<FLOAT> &dropoutMask, const FLOAT dropoutRate, PStream &stream)
{
    ...
    verify(curandSetStream(curandGen, stream.cudaStream) == CURAND_STATUS_SUCCESS);
    verify(RANDU(curandGen, ptrMask, n) == CURAND_STATUS_SUCCESS);
