...
enum ActType {ACT_BIAS, ACT_SIGMOID, ACT_TANH, ACT_SOFTPLUS, ACT_RECTLINEAR, ACT_LINEAR, ACT_ONE_MINUS_LINEAR, ACT_INVERSE, ACT_SOFTMAX, ACT_DROPOUT};
...
{
    public:
        LayerParam() : initVal((FLOAT) 0), dropoutRate((FLOAT) 0), statePenalty(NO_STATE_PENALTY) {}
...
public:
	...
	void SetStatePenalty(const FLOAT val);
        ...
        void GenerateDropoutMask(const unsigned long batchFrom, const unsigned long batchTo);
...
protected:
	...
	Matrix<FLOAT> act_fixed, state_fixed, act, state, srcErr, dstErr;
